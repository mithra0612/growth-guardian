    ...
    setTimeout(() => {
      ...
      if (formData.type === "investment") {
        ...
        if (additional > 0) {
          ...
          for (let i = 1; i <= periods; i++) {
            futureValue +=
              additional *
              monthlyFactor *
              Math.pow(1 + periodRate, periods - i);
          }
        }
        ...
        const interest =
          futureValue -
          principal -
          additional * periods * (n === 12 ? 1 : n === 4 ? 3 : 12);
        calculation = {
      ...
      <div className="max-w-3xl w-full bg-white rounded-xl shadow-lg p-8 border border-blue-100">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">
            Calculate investments and loans in Indian Rupees (₹)
          </p>
        </div>
        ...
        <form onSubmit={calculateResults} className="space-y-6">
          ...
          >
            {isCalculating ? (
              ...
              `Calculate ${
                formData.type === "investment" ? "Returns" : "Payments"
              }`
            )}
          </button>
        </form>
        ...
        {showModal && result && (
          <div className="fixed inset-0 bg-gray-900/70 flex items-center justify-center p-6 z-50">
            <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
              <div
                className={`w-16 h-16 mx-auto -mt-10 rounded-full flex items-center justify-center text-white text-2xl ${
                  formData.type === "investment" ? "bg-blue-500" : "bg-blue-500"
                }`}
              >
                {formData.type === "investment" ? "₹+" : "₹-"}
              </div>
