...
const FloatingChatbot = () => {
  ...
  const [messages, setMessages] = useState([
    { text: "Hello! I can help you with information about banking. What would you like to know?", sender: 'bot' },
  ]);
  ...
  const fetchBotResponse = async (userPrompt) => {
    setIsLoading(true);
    try {
      const response = await fetch('https://hackit-fin-tech-backend.vercel.app/api/chatBot', {
        method: 'POST',
        headers: {
  ...
  const handleSend = async () => {
    if (message.trim()) {
      ...
      setMessages(prev => [...prev, { text: botData.response || "I didn't get a response. Please try again.", sender: 'bot' }]);
    }
  };
    ...
    <div className="fixed bottom-8 right-8 z-50 font-sans">
      ...
      <button
        ...
        className={`flex items-center justify-center w-16 h-16 rounded-full shadow-lg transition-all duration-300 hover:shadow-xl ${
          isOpen ? 'bg-white text-blue-600 rotate-90' : 'bg-blue-600 text-white hover:bg-blue-700'
        }`}
      >
